...
module AVal =
    ...
    let private averageDeltas =
        System.Runtime.CompilerServices.ConditionalWeakTable<aval<DateTime>, System.Collections.Concurrent.ConcurrentQueue<float>>()

    let private averageTimeQueue (time : aval<DateTime>) =
        lock averageDeltas (fun () ->
            match averageDeltas.TryGetValue time with
            | (true, q) -> q
            | _ ->
                let q = System.Collections.Concurrent.ConcurrentQueue<float>()
                averageDeltas.Add(time, q) |> ignore
                q
        )
    
    let stepTimeNew (action : DateTime -> TimeSpan -> 'a -> 'a) (time : aval<DateTime>) : AdaptiveFunc<'a> =
        let queue = averageTimeQueue time
        let mutable tLast = None
        AdaptiveFunc<'a>(fun token value ->
            let now = time.GetValue token
            let struct(tl, dt) =
                match tLast with
                | Some tl -> 
                    let dt : TimeSpan = now - tl
                    queue.Enqueue dt.TotalSeconds
                    while queue.Count > 10 do queue.TryDequeue() |> ignore
                    struct(tl, dt)
                | None -> 
                    let dt = 
                        if queue.Count > 0 then TimeSpan.FromSeconds (Seq.average queue)
                        else TimeSpan.Zero
                    struct(now - dt, dt)
            tLast <- Some now
            action tl dt value
...
module CameraController =
    ...
    let controlWSAD (keyboard : IKeyboard) (time : aval<DateTime>) =   
        ...
        adaptive {
            ...
            if m <> V2i.Zero then
                ...
                return time 
                |> AVal.stepTimeNew (fun t dt (cam : CameraView)  ->
    ...
    let controllScroll (mouse : IMouse) (time : aval<DateTime>) =
        ...
        adaptive {
            let! a = active 
            if a then
                return time |> AVal.stepTimeNew (fun t dt (cam : CameraView) ->
